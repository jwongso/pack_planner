...
const std::vector<sort_order> benchmark::SORT_ORDERS = {sort_order::NATURAL, sort_order::LONG_TO_SHORT, sort_order::SHORT_TO_LONG};
...
void benchmark::run_benchmark() {
    ...
    std::cout << "Total benchmark execution: " << std::fixed << std::setprecision(3) 
              << total_benchmark_time << " ms (" << static_cast<long long>(total_benchmark_time * 1000) << " μs)" << std::endl;
}
...
benchmark_result benchmark::run_single_benchmark(int size, sort_order order) {
    ...
    if (result.total_time > 0) {
        result.items_per_second = static_cast<int>((plan_result.total_items * 1000.0) / result.total_time);
    } else {
